...
function Notes() {
  ...
  return (
    <div className='max-w-2xl my-5 mx-auto'>
        { notes.map(note => (          
          <NoteItem key={note} id={note} content={note} />           
        ))}
    </div>
  )
}
...
function NoteItem( {content , id} ) {
  ...
  return  <div className='border-b-1 border-gray-500 py-2 flex justify-between items-center '>                
    ...
    
    {
      loading ?  (
        <div role="status">
            <svg className="w-4 h-4 me-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <button className="flex-shrink-0 text-red-500 opacity-70 hover:opacity-100 ml-2 transition-opacity" onClick={() => deleteNote(id)} disabled={loading}>
